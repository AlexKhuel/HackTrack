...
1. Pulls in-person hackathons from MLH and Devpost
...
create table events (
  ...
  source             text,      -- 'mlh' | 'devpost' | 'mlh,devpost'
  name               text,
  city               text,
  country            text,
...
Triggers a background re-scrape of MLH and Devpost. Requires `Authorization` header. Returns `202 Accepted` immediately.
...
MLH and Devpost are scraped, merged, deduplicated, and loaded into Supabase on a schedule. GitHub Actions triggers a sync every Sunday and Wednesday at 00:00 UTC, or on demand via the admin endpoint.
