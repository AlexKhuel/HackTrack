...
create table public."user" (
  ...
  secondary_airport_code text,
  tertiary_airport_code text,
  friend_cities text[] not null default array[]::text[],
  timezone text,
  max_cost smallint default 1000 check (max_cost >= 0),
  max_time smallint default 1000 check (max_time >= 0),
  friday_last_class time,
  monday_first_class time
);
```
...
Exchanges a Google ID token (`credential`) for a server-issued session token.

### `GET /api/auth/me`

Returns the authenticated user profile for a bearer token.

### `POST /api/user-inputs`

Upserts the signed-in user's current form values into `public."user"`.

### `GET /api/user-inputs/latest`

Returns the latest saved form payload for the signed-in user.
